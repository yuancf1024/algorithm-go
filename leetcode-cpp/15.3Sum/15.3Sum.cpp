...
public:
    vector<vector<int>> threeSum(vector<int>& nums) {
        ...
        for (int i = 0; i < nums.size(); i++) {
            // 排序之后如果第一个元素已经大于0，那么无论如何组合都不可能凑成三元组，直接返回结果
            if (nums[i] > 0) {
                return result;
            }
...
而且使用哈希法 在使用两层for循环的时候，能做的剪枝操作很有限，
虽然时间复杂度是O(n^2)，也是可以在leetcode上通过，
但是程序的执行时间依然比较长 。
