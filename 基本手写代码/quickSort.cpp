 ...
 * 
 * 解释时间复杂度为nlogn
 * 分治算法，每次选出哨兵，将待排序列分成两部分，一直分下去，
 * 直到只有一个元素，平均分开，相当于n个结点的二叉树，共有logn层，
 * 递归深度为logn
 * 每一层都需要对当前分组的元素排序，问题规模大概为n，
 * 第一层遍历n个元素，第二层遍历n-1个...直到最后一层为一个
 * nlogn
 * 
 * 解释空间复杂度O(logn)~O(n)
 * 主要是递归造成的栈空间的使用
 ...
 * 
 * 快速排序，平均情况O(nlogn),最好O(nlogn)，
 * 最坏O(n^2)（选择标定元素有关），空间复杂度O(logn)~O(n)，不稳定
 * 
 */
...
int partition2(int *data, int start, int end) {
    ...
    while (true) {
        ...
        // 下面这两行，swap 交换之后，双方需要移动，否则会增加一次无用的比较
        i++;
        j--;
    }
...
int partition(int *data, int start, int end) {
    ...
    // 最后将start放在应该的位置，即small和small+1之间，
    // 因为左侧全是小于pivot的，因此将small和pivot交换
...
int main() {
    ...
    quickSortOffer(arr, 0, 7); // 0 表示从数组0位置开始，到4位置排序
    int i;
    for (i = 0; i < 8; ++i) {
        printf("%d ", arr[i]);
    }
